    ...
    <nav class="fixed top-0 left-0 right-0 z-50 transition-all duration-500" [ngClass]="navClass">
      <div class="max-w-6xl mx-auto px-6 md:px-8 flex items-center justify-between h-16">
        ...
        <div class="flex items-center gap-3">
          ...
          <a href="#"
             class="hidden sm:inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium transition-all duration-300"
             [ngClass]="dark ? 'bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] text-white hover:opacity-95 shadow-[0_10px_30px_rgba(99,102,241,0.25)]' : 'bg-gradient-to-r from-[#4f46e5] to-[#7c3aed] text-white hover:opacity-95 shadow-[0_10px_30px_rgba(79,70,229,0.18)]'">
            <span class="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-white/15">PRO</span>
            <span>Get Pro</span>
          </a>
        </div>
      </div>
